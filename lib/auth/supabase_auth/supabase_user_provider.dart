...
class RotaNewSupabaseUser extends BaseAuthUser {
  RotaNewSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> rotaNewSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = RotaNewSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
